...
pub async fn revoke_identity_link(
    ...
    if let Some(agent_key) = agent_key {
        let _ = notify_vault_revoke("Kindact", &agent_key).await;
    }
...
///
/// Export this user's Kindact data for Vault auto-backup.
...
/// agent plus every other Kindact agent linked to the same Flowsta Vault
...
/// observable in kindact.log (webview console logs aren't readable on disk).
///
...
//
// Adding a new entry type to Kindact means adding ONE `match` arm in
...
pub async fn build_canonical_backup(
    ...
    let mut payload = serde_json::json!({
        "version": 1,
        "_readme": "Your Kindact data, backed up automatically by Flowsta Vault. Encrypted with your device key — only you can read it. Each record below carries a plain-English view of what you authored AND a signed Holochain record for restore. The lair_* fields are the cryptographic keys that let you recover this identity on a fresh install.",
