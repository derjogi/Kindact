...
mod win_job {
    ...
    //! `PR_SET_PDEATHSIG`. Every sidecar (`kindact-holochain`,
    //! `kindact-lair-keystore`) is assigned to one process-wide job that has
...
mod windows_hide {
    ...
    const SIDECAR_TITLE_MARKERS: &[&str] = &["kindact-holochain", "kindact-lair-keystore"];
