//! Holochain conductor lifecycle management for Kindact.
//!
...
//
//   KINDACT_BOOTSTRAP_URL=https://your-bootstrap.example.com   \
//   KINDACT_SIGNAL_URL=wss://your-bootstrap.example.com        \
//   KINDACT_RELAY_URL=https://your-bootstrap.example.com./     \
//   KINDACT_AUTH_MATERIAL=<standard base64 of opaque auth bytes>     \
//     cargo tauri build
//
// `KINDACT_AUTH_MATERIAL` is optional and only set when targeting a
...
/// Override with `KINDACT_BOOTSTRAP_URL` for production.
...
/// `KINDACT_RELAY_URL` for production.
...
/// fork's CI referencing `${{ secrets.KINDACT_BOOTSTRAP_URL }}` when
...
fn bootstrap_url() -> &'static str {
    env_or!("KINDACT_BOOTSTRAP_URL", DEFAULT_BOOTSTRAP_URL)
}

fn signal_url() -> &'static str {
    env_or!("KINDACT_SIGNAL_URL", DEFAULT_SIGNAL_URL)
}

fn relay_url() -> &'static str {
    env_or!("KINDACT_RELAY_URL", DEFAULT_RELAY_URL)
}

fn auth_material() -> Option<&'static str> {
    match option_env!("KINDACT_AUTH_MATERIAL") {
...
/// Generate conductor-config.yaml for Kindact.
...
fn start_conductor_process(
    ...
    let mut child = std::process::Command::new(sidecar_path("kindact-holochain"))
        .arg("-c")
        .arg(config_path)
        .arg("--piped")
        .stdin(Stdio::piped())
...
///
/// Kindact uses lair's auto-generated key (no deterministic seed import).
