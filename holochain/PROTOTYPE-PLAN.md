...
**Status update (spec [049](specs/049-prototype-anchor-subscription-discovery/README.md))**: Anchor-driven cross-cell discovery is now the primary path in `kindact-hc/`. On every `create_issue` / `create_housing_issue`, the cell cross-cell-calls `global_registry::publish_anchor_link` for each user-supplied tag. Subscriptions are persisted on the agent's `global_registry` source chain via `subscribe` / `unsubscribe` / `get_subscriptions` externs. The UI's lens manager now writes those subscriptions; the discovery feed is rebuilt from `(get_subscriptions ∘ get_anchor_links_for_anchor)` and dereferenced into the owning cell — unless the `#general` lens is on, in which case the old per-cell enumeration acts as a "show everything" fallback. This delivers most of Phase 1 question 1: a subscriber to `#wind-power` sees a Manhattan-cell issue without having authored it themselves.

**Status update (spec [050](specs/050-prototype-dynamic-cell-creation-and-join/README.md))**: Cells are no longer hardcoded in `workdir/happ.yaml`. The `manhattan_windturbine` role now has `clone_limit: 16` and the UI surfaces a Communities panel: any agent can create a new community cell (`createCloneCell` + register in `global_registry`), and any other agent sees that cell in their directory within one poll tick and can Join it (`createCloneCell` with the same network seed). Anchor links carry the origin `cell_dna_hash` so subscription-driven discovery dereferences into the correct local clone; an issue from a not-yet-joined cell is surfaced as a "join to view" placeholder. The full create → register → discover → join → write → comment loop is real on the prototype. This closes the "Cell installation per persona" open question from 049 in the cell-create direction (per-persona role *omission* at conductor-startup time remains a Holo-hosting follow-up).
